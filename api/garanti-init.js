...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const callbackUrl = `${origin}/api/garanti-callback?oid=${encodeURIComponent(orderId)}`;
    ...
    // Garanti e-ticaret destek için request kaydı (şifre/storeKey formda yok)
    console.log(
      '[garanti-init] REQUEST',
      JSON.stringify({
        action: form.action,
        mode: cfg.mode,
        ...
        passwordLen: String(cfg.provPassword || '').length,
        storeKeyLen: String(cfg.storeKey || '').length,
        storeKeyIs24Hex:
          String(cfg.storeKey || '').length === 24 &&
          /^[0-9a-fA-F]+$/.test(String(cfg.storeKey || '')),
        orderId,
        amountKurus: paymentAmount,
        installmentForm: form.fields.txninstallmentcount,
        hashLen: String(form.fields.secure3dhash || '').length,
        fields: form.fields,
      })
    );
