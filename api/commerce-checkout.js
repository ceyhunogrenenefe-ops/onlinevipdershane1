/**
 * Kitap mağazası checkout köprüsü (koçluk paneli → Garanti / Ziraat / PayTR)
 * POST { op: 'resolve' | 'pay', token, customer?, address?, provider? }
 */
const {
  paytrConfig,
  buildUserBasket,
...
const { ziraatConfig, buildHostingPaymentForm } = require('./_lib/ziraat');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (op === 'confirm_paid') {
      const data = await panelCommerce('checkout.confirm_paid', { token });
    ...
    if (op === 'pay') {
      ...
      const provider = String(body.provider || 'garanti').toLowerCase();
      ...
      if (provider === 'garanti') {
        ...
        const callbackUrl = `${origin}/api/garanti-callback?source=kitap&oid=${encodeURIComponent(merchantOid)}`;
      ...
      if (provider === 'ziraat') {
        ...
        if (!cfg) {
          return res.status(500).json({
            error:
              'Ziraat Sanal POS yapılandırılmamış. Vercel ortam değişkenlerine ZIRAAT_CLIENT_ID ve ZIRAAT_STORE_KEY ekleyin.',
          });
        }
        const form = buildHostingPaymentForm({
          cfg,
          orderId: merchantOid.slice(0, 64),
          amountKurus: paymentAmount,
          email,
          billToName: parentName,
          successUrl: `${origin}/api/ziraat-callback?result=ok&source=kitap&oid=${encodeURIComponent(merchantOid)}`,
          errorUrl: `${origin}/api/ziraat-callback?result=fail&source=kitap&oid=${encodeURIComponent(merchantOid)}`,
          installmentCount: String(body.installment || '').replace(/\D/g, ''),
          lang: 'tr',
        });
        return res.status(200).json({
